...
def make_stgf(E0, E1, dE, chanorder, sym, nch, run = True) :
  j,pi = symb_jpi(sym)
...
open_ctrl={'0o':{'2ch':{'E': [-0.27,0.05,0.001], 'order' : [1,5]},
         ...
         '1o':{
               ...
               '6ch':{'E':[-0.15,0.10,0.001],'order':[1,2,3,7,10,13]},
...
for iax, isym in enumerate(symms) :
  if (isym not in open_ctrl) : continue
  elif (calc == 'update') :
    channels = get_channel('./'+isym)
  elif (calc == 'ctrl_list') :
    channels = ["./{}/{}".format(isym,i) for i in open_ctrl[isym].keys()] 
  ...
  for isec, ich in enumerate(channels) : 
    print(open_ctrl[isym])
    ch_name = ich.split('/')[2]
    print(ch_name)
    if ch_name not in open_ctrl[isym] : continue
    [E0,E1,dE] = open_ctrl[isym][ch_name]['E']
    chanorder = open_ctrl[isym][ch_name]['order']
    print(isym, ich, ch_name, E0,E1,dE)
    make_stgf(E0, E1, dE, chanorder, isym, ch_name, run = True)
    #exit()
