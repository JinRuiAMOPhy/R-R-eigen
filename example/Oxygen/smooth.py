...
action = 'plot, smooth:2o-mqdt/all' #'6ch,10ch,14ch,19ch,22ch,25ch,28ch' #'smooth;plot'
Z = 1.0
#symms = ['0o','0e', '0e-1o', '1e-1o', '2e-1o', '1e', '2o','2e'] 
#symms = ['1S0e-1o', '1D2e-1o', '3P2e-1o', '3P1e-1o', '3P0e-1o']
symms = [ '2o-mqdt'] 
...
for isym in symms :
  ...
  if not sym_f in symm_f_list : symm_f_list.append(sym_f)

print(symm_f_list,len(symm_f_list))
fig = plt.figure(figsize= figsize )
gs = fig.add_gridspec(len(symm_f_list),1, hspace=0.3)
...
iax = -1
do_plot = True #False
for ixx, isym in enumerate(symms) :
  if '-' in isym : sym_f = isym.split('-')[1]
  else : sym_f = isym
  channels = get_channel('./'+isym)
  print('final_sym', sym_f)
  print(channels )#, maxchan)
  maxchan = max([int(ich.split('/')[2].split('ch')[0]) for ich in channels])
  if sym_f in test : do_plot = False
  else : 
    do_plot = True
    test.append(sym_f)
    iax = iax + 1
  print('plot ', do_plot,'iax',iax, 'sym_f', sym_f)
  if len(symm_f_list) > 1 : ax = axs[iax]
  else : ax = axs
  for isec, ich in enumerate(channels) : 
    print('ich',ich,ich.split('/')[2])
    if 'smooth' in action and \
         ((ich.split('/')[2] in action ) or 'all' in action) and sym_f in action :
      print(ich)
    ...
    if isec == 0 :
      ...
      tidy_axis(**ax_dict)
      text = '({})'.format(chr(ord('a') + iax)) +'symmetry :' +sym_f; 
      tidy_text(text, **ax_dict)

plt.savefig('miu-extend.pdf', format = 'pdf')
