...
! print with advane = 'no' in the end
   ...
   subroutine helper
      ...
      call explain_something_colon(stdout,'',&
           'os sort interp levfind deriv(or dtaudE)',20)
      call explain_something_colon(stdout,'',&
           'one can combine options by e.g. -debug levfind+deriv',20)
      call explain_something_colon(stdout,'',&
           'or simply -debug all to print them all', 20)
      call explain_something_colon(stdout,'-EG',&
           'energy of intial state photon absorption', 20)
      call explain_something_colon(stdout,'',&
           'default unit ry. one can specify other unit by', 20)
      call explain_something_colon(stdout,'',&
           'e.g. 0.2cm-1, 0.2au 0.2ev 0.2Ryd 0.2Hartree', 20)
      call explain_something_colon(stdout,'-filter',&
           'autoionization resonance, threshold for detecting peaks', 20)
      call explain_something_colon(stdout,'',&
           'of dtau/dE, generally no need to specify it,', 20)
      call explain_something_colon(stdout,'',&
           'the dtau/dE will be internally computed', 20)
