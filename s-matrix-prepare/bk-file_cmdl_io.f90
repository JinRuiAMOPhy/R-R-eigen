...
contains
   ...
   subroutine run_dfde(FN, CTR, T, S)
      implicit none
   ...
   subroutine read_mqdt_input_key(FN, CTR, T, S, G, keys, args)
      ...
      do i = 1, size(keys)
         ...
         if(keys(i) == 'z') then
            ...
            allocate(T%IP_seq(ncol))
            read(vals(i), *)(T%IP_seq(j), j = 1, ncol)
            ...
            allocate(S%d_mu(ncol))
            read(vals(i), *)(S%d_mu(j), j = 1, ncol)
